(() => {
    ...
    __webpack_require__.r = exports => {
        ...
        const GenericErrorMessage = "An unexpected error occured. Please refresh the page and try again.";
        ...
        var obj, key, value, css_alert = __webpack_require__(39), css_chat = __webpack_require__(5644), browse = __webpack_require__(3630);
        function browseInjected(browseHtml) {
            return `\n    <style>\n      ${browse}\n    </style>\n    ${browseHtml}\n  `;
        }
        class DisneyChatApi extends class {
            constructor() {
            ...
            _injectChat() {
                this._chatHtml && jQuery(DisneyChatApi.MAIN_WRAPPER_SELECTOR).after(function(chatHtml) {
                    return `\n    <style>\n      ${css_alert}\n    </style>\n\n    <style tpInjected>\n      .with-chat {\n        left: 0px !important;\n        width: calc(100% - 288px) !important;\n      }\n\n      .tp-video {\n        transition: width 250ms linear 0.2s;\n      }\n\n      ${arguments.length > 1 && void 0 !== arguments[1] ? arguments[1] : ""}\n    \n      ${css_chat}\n      \n    </style>\n\n    ${chatHtml}\n  `;
                }(this._chatHtml));
            }
            getChatVisible() {
                return jQuery(DisneyChatApi.MAIN_WRAPPER_SELECTOR).hasClass("with-chat");
            }
            setChatVisible(visible) {
                super.setChatVisible(visible), visible ? (jQuery("#chat-wrapper").show(), jQuery(DisneyChatApi.MAIN_WRAPPER_SELECTOR).addClass("with-chat"), 
                document.hasFocus() || this.clearUnreadCount()) : (jQuery("#chat-wrapper").hide(), 
                jQuery(DisneyChatApi.MAIN_WRAPPER_SELECTOR).removeClass("with-chat"));
            }
            removeChat() {
                super.removeChat(), jQuery(DisneyChatApi.MAIN_WRAPPER_SELECTOR).removeClass("with-chat");
            }
        }
        ...
        value = "#hudson-wrapper", (key = "MAIN_WRAPPER_SELECTOR") in (obj = DisneyChatApi) ? Object.defineProperty(obj, key, {
            value,
            enumerable: !0,
            configurable: !0,
            writable: !0
        }) : obj[key] = value;
        let HboVideoType, PlaybackState, SessionState, StreamingServiceName;
        ...
        }(HboVideoType || (HboVideoType = {})), function(PlaybackState) {
        ...
        class DisneyVideoApi extends class {
            constructor() {
            ...
            constructor() {
                ...
                }(this, "_video", void 0), this._uiEventsHappening = 0, console.log("Diseny video");
            }
            getStreamingServiceName() {
                return StreamingServiceName.DISNEY_PLUS;
            }
            async waitVideoReady() {
                if (!this._video) try {
                    ...
                    await delayUntil((() => {
                        ...
                        return video instanceof Element && video.readyState > 0;
                    }), 8e3)(), this._video = null !== (_document$querySelect = document.querySelector("video")) && void 0 !== _document$querySelect ? _document$querySelect : void 0;
                } catch (error) {
            ...
            _getSeekbar() {
                const seekbar = document.querySelector(".progress-bar .slider-container");
                if (seekbar && seekbar instanceof HTMLElement) return seekbar;
            }
            _getMousePositionFromProgress(progress) {
                const seekbar = this._getSeekbar();
                if (seekbar) {
            ...
            async waitVideoApiReadyAsync() {
                this._video || await this.waitVideoReady();
            }
            waitVideoDoneLoadingAsync() {
                return delayUntil((() => this._getPlaybackState() !== PlaybackState.LOADING), 1e4)();
            }
            getStateAsync() {
                return new Promise(((resolve, reject) => {
                    const playbackState = this._getPlaybackState(), playbackPositionMilliseconds = this._getPlaybackPositionMilliseconds();
                    void 0 !== playbackPositionMilliseconds ? resolve({
                        playbackState,
            ...
            getVideoDataAsync() {
                return new Promise(((resolve, reject) => {
                    const videoDuration = this._getDuration(), videoId = this._getVideoId(), videoTitle = this._getVideoTitle();
                    void 0 !== videoDuration && void 0 !== videoId && void 0 !== videoTitle ? resolve({
                        videoId,
                        videoTitle,
                        videoDuration
                    }) : reject();
            ...
            async jumpToNextEpisode(nextEpisodeMessageData) {
                ...
                (null !== (_window$location$href = window.location.href.match(/^.*\/([0-9a-zA-Z-]+)\??.*/)) && void 0 !== _window$location$href ? _window$location$href : [ void 0, void 0 ])[1] != nextEpisodeMessageData.videoId && window.postMessage({
                    type: "NEXT_EPISODE",
            ...
            async pause() {
                this._uiEventsHappening += 1;
                try {
                    jQuery(".btm-media-player").trigger("click"), await delay(100)(), jQuery(".pause-icon").length > 0 && jQuery(".pause-icon").trigger("click"), 
                    await delayUntil((() => this._getPlaybackState() === PlaybackState.PAUSED), 1e3)();
                } catch (error) {
                    if (this._getPlaybackState() !== PlaybackState.PAUSED) throw error;
                } finally {
                    this._uiEventsHappening -= 1;
                }
            }
            async play() {
                this._uiEventsHappening += 1;
                try {
                    jQuery(".btm-media-player").trigger("click"), await delay(100)(), jQuery(".play-icon").length > 0 && jQuery(".play-icon").trigger("click"), 
                    await delayUntil((() => this._getPlaybackState() === PlaybackState.PLAYING), 1e3)();
                } catch (error) {
                    if (this._getPlaybackState() !== PlaybackState.PLAYING) throw error;
                } finally {
                    this._uiEventsHappening -= 1;
                }
            }
            async setCurrentTime(time) {
                debug("Seeking: " + time), this._uiEventsHappening += 1;
                try {
                    this._showUIAsync(), await delay(140)();
                    const video = this.getVideoElement(), seekbar = this._getSeekbar();
                    if (seekbar && video) {
                        var _this$_getMousePositi;
                        const progress = time / (1e3 * video.duration), x = null !== (_this$_getMousePositi = this._getMousePositionFromProgress(progress)) && void 0 !== _this$_getMousePositi ? _this$_getMousePositi : 0;
                        this._clickAtX(seekbar, x, "pointerdown"), this._clickAtX(seekbar, x, "pointerup"), 
            ...
            _getPlaybackState() {
                const video = this.getVideoElement();
                return null == video ? PlaybackState.NOT_READY : video.readyState < video.HAVE_FUTURE_DATA ? PlaybackState.LOADING : video.paused ? PlaybackState.PAUSED : PlaybackState.PLAYING;
            }
            ...
            _getVideoId() {
                var _DisneyPlus$getVideoI;
                return null !== (_DisneyPlus$getVideoI = Disney.getVideoId(new URL(window.location.href))) && void 0 !== _DisneyPlus$getVideoI ? _DisneyPlus$getVideoI : "";
            }
            _getVideoTitle() {
                return "";
            }
            async getUpdateSessionDataAsync() {
                const currentTime = this._getPlaybackPositionMilliseconds();
        ...
        function DisneyVideoEventListener_defineProperty(obj, key, value) {
            return key in obj ? Object.defineProperty(obj, key, {
                value,
                enumerable: !0,
                configurable: !0,
        ...
        class DisneyVideoEventListener extends class {
            ...
            constructor(videoApi, chatApi, pageControls) {
                ...
                super(videoApi), DisneyVideoEventListener_defineProperty(this, "_chatApi", void 0), 
                DisneyVideoEventListener_defineProperty(this, "_videoApi", void 0), DisneyVideoEventListener_defineProperty(this, "_pageControls", void 0), 
                DisneyVideoEventListener_defineProperty(this, "_wrapperObserver", void 0), DisneyVideoEventListener_defineProperty(this, "_loadingObserver", void 0), 
                DisneyVideoEventListener_defineProperty(this, "_onUpdate", this._onVideoUpdate.bind(this)), 
                DisneyVideoEventListener_defineProperty(this, "_onReplace", this.replaceStateInteraction.bind(this)), 
                this._videoApi = videoApi, this._chatApi = chatApi, this._pageControls = pageControls, 
                this._wrapperObserver = new MutationObserver((mutationsList => {
                    for (const mutation of mutationsList) "class" === mutation.attributeName && mutation.target.classList.contains("video_view--theater") && this._chatApi.setChatVisible(this._chatApi.shouldChatBeVisible());
                })), this._loadingObserver = new MutationObserver((mutationsList => {
                    for (const mutation of mutationsList) {
                        if (mutation.addedNodes && mutation.addedNodes.length > 0) {
                            jQuery(mutation.addedNodes).find(".loading-icon").length && this._onVideoBuffering();
                        }
                        if (mutation.removedNodes && mutation.removedNodes.length > 0) {
                            jQuery(mutation.removedNodes).find(".loading-icon").length && this._onVideoCanPlay();
                        }
                    }
                })), null !== (_window$teleparty = window.teleparty) && void 0 !== _window$teleparty && _window$teleparty.replaceScriptLoaded || (debug("injecting replace script"), 
                function(script) {
                ...
                null !== (_window$teleparty2 = window.teleparty) && void 0 !== _window$teleparty2 && _window$teleparty2.injectScriptLoaded || function(scriptLocation) {
                    const s = document.createElement("script");
                    s.setAttribute("tpInjected", ""), s.src = scriptLocation, (document.head || document.documentElement).appendChild(s), 
                    s.remove();
                }(chrome.extension.getURL("content_scripts/disney/disney_injected_bundled.js"));
            }
            reloadListeners() {
                ...
                video.addEventListener("seeking", this._onUpdate)), this._pageControls.reloadListeners();
            }
            startListening() {
                const config = {
                    attributes: !0
                }, targetNode = document.getElementById("hudson-wrapper"), overlayContainer = document.getElementsByClassName("btm-media-overlays-container")[0];
                this._wrapperObserver.disconnect(), this._loadingObserver.disconnect(), targetNode && this._wrapperObserver.observe(targetNode, config), 
                overlayContainer && this._loadingObserver.observe(overlayContainer, {
                    childList: !0
                });
                const video = this._videoApi.getVideoElement();
                video && (video.addEventListener("play", this._onUpdate), video.addEventListener("pause", this._onUpdate), 
                video.addEventListener("seeking", this._onUpdate)), window.addEventListener("message", this._onReplace, !1);
            }
            getVideo() {
            ...
            stopListening() {
                this._wrapperObserver.disconnect(), this._loadingObserver.disconnect();
                const video = this._videoApi.getVideoElement();
            ...
            async loadNewVideoAsync(videoId) {
                ...
                await new Promise(((resolve, reject) => {
                    const interval = setInterval((() => {
                        if (Disney.getVideoId(new URL(window.location.href)) === videoId) {
                            const videoElement = this._videoApi.getVideoElement();
                            if (videoElement instanceof Element && videoElement.src) return debug("Loaded new Disney video"), 
                            clearInterval(interval), void resolve();
                        }
                        performance.now() - start >= 45e3 && (clearInterval(interval), reject(new Error("Could not load new video in time.")));
                    }), 100);
            ...
            replaceStateInteraction(event) {
                if (event.source == window) if ("FROM_PAGE_POP" !== event.data.type) {
                    if (event.data.type && "FROM_PAGE" == event.data.type) {
                        ...
                        const episodePage = window.location.href.match(/^.*\/(video)\/.*/);
                        if (debug(window.location.href), episodePage) {
                            var _this$_videoMessageFo;
                            const nextVideoId = Disney.getVideoId(new URL(window.location.href));
                            if (!nextVideoId) return debug("No video found. Tearing down"), void this._onTeardown(DEFAULT_TEARDOWN);
                            nextVideoId !== (null === (_this$_videoMessageFo = this._videoMessageForwarder) || void 0 === _this$_videoMessageFo ? void 0 : _this$_videoMessageFo.videoId) && this._onNextEpisode(nextVideoId);
        ...
        class GetSessionDataMessage extends ClientMessage {
            constructor(sender, target, data) {
                super(sender, target, ClientMessageType.GET_SESSION_DATA), function(obj, key, value) {
                    key in obj ? Object.defineProperty(obj, key, {
                        value,
                        enumerable: !0,
                        configurable: !0,
                ...
                }(this, "data", void 0), this.data = data;
            }
        }
        ...
        class BroadcastMessage extends ClientMessage {
            constructor(sender, target, data) {
                super(sender, target, ClientMessageType.BROADCAST), function(obj, key, value) {
                    key in obj ? Object.defineProperty(obj, key, {
                        value,
                        enumerable: !0,
                        configurable: !0,
        ...
        class VideoApiMessage extends Message {
            constructor(sender, target, type) {
                super(sender, target, type), function(obj, key, value) {
                    key in obj ? Object.defineProperty(obj, key, {
                        value,
                        enumerable: !0,
                        configurable: !0,
        ...
        class BroadcastNextEpisodeMessage extends ClientMessage {
            constructor(sender, target, data) {
                super(sender, target, ClientMessageType.BROADCAST_NEXT_EPISODE), function(obj, key, value) {
                    key in obj ? Object.defineProperty(obj, key, {
                        value,
                        enumerable: !0,
                        configurable: !0,
        ...
        class SetBufferingMessage extends ClientMessage {
            constructor(sender, target, data) {
                super(sender, target, ClientMessageType.SET_BUFFERING), function(obj, key, value) {
                    key in obj ? Object.defineProperty(obj, key, {
                        value,
                        enumerable: !0,
                        configurable: !0,
        ...
        class SetWatchingAdsMessage extends ClientMessage {
            constructor(sender, target, data) {
                super(sender, target, ClientMessageType.SET_WATCHING_ADS), function(obj, key, value) {
                    key in obj ? Object.defineProperty(obj, key, {
                        value,
                        enumerable: !0,
                        configurable: !0,
        ...
        class CreateSessionMessage extends PopupMessage {
            constructor(sender, target, data) {
                super(sender, target, PopupMessageType.CREATE_SESSION), function(obj, key, value) {
                    key in obj ? Object.defineProperty(obj, key, {
                        value,
                        enumerable: !0,
                        configurable: !0,
        ...
        class ReInjectMessage extends PopupMessage {
            constructor(sender, target, data) {
                super(sender, target, PopupMessageType.RE_INJECT), function(obj, key, value) {
                    key in obj ? Object.defineProperty(obj, key, {
                        value,
                        enumerable: !0,
                        configurable: !0,
        ...
        class DisneyPageControls extends class {
            constructor(chatApi) {
                ...
                console.log("Teleparty Browse Loaded " + (null === (_window$teleparty = window.teleparty) || void 0 === _window$teleparty ? void 0 : _window$teleparty.tabId)), 
            ...
            hideMessageIndicator() {
                jQuery("#tp-message-indicator").addClass("hidden"), jQuery("#tp-chat-gray").addClass("hidden"), 
                jQuery("#tp-chat-hidden").removeClass("hidden");
            }
            onChatMessage() {
                this._unopenedMessageCount++, this.showMessageIndicator(), this.showTpIcon();
            }
            teardown() {
            ...
            hideTpIcon() {
                this.hideError(), jQuery("#tpIconContainer").addClass("hidden");
            }
            async showTpIcon() {
                if (0 === jQuery("#tpIconContainer").length) {
                    const sideHtml = this.loadImages(browsePopup_default()), controlsRoot = await this.getControlsRoot();
                    controlsRoot.length > 0 && controlsRoot.append(browseInjected(sideHtml)), this.loadHostOnlyImage(), 
                    this._setupEventHandlers();
                }
                this._chatAPI.inSession ? this.enablePartyIcons() : this.disablePartyIcons(), jQuery("#tpIconContainer").removeClass("hidden"), 
                this.setChatButtons();
            }
            getControlsHeight() {
                return "100px";
            ...
            async onToggleChat() {
                this._chatAPI.setChatVisible(!this._chatAPI.shouldChatBeVisible()), this._unopenedMessageCount = 0, 
                this.hideMessageIndicator(), this.setChatButtons();
            }
            setChatButtons() {
                this._chatAPI.inSession && this._chatAPI.shouldChatBeVisible() ? this.showChatOpenButton() : this.hideChatOpenButton();
            }
            async showChatOpenButton() {
                await delay(250)(), jQuery("#tp-chat-close-button").removeClass("hidden"), jQuery("#tp-buttons-container").addClass("hidden");
            }
            hideChatOpenButton() {
            ...
            constructor() {
                super(...arguments), function(obj, key, value) {
                    key in obj ? Object.defineProperty(obj, key, {
                        value,
                        enumerable: !0,
                        configurable: !0,
                ...
                }(this, "_controlsObserver", void 0);
            }
            async getControlsRoot() {
                return jQuery("#hudson-wrapper");
            }
            startEventListener() {
                console.log("Started"), this._controlsObserver = new MutationObserver((mutationsList => {
                    for (const mutation of mutationsList) "class" === mutation.attributeName && mutation.target.classList.contains("overlay__controls--visually-hide") ? this.hideTpIcon() : "class" === mutation.attributeName && mutation.target.classList.contains("overlay__controls--visually-show") && this.showTpIcon();
                }));
                const overlay = jQuery(".overlay__controls");
                this._controlsObserver.observe(overlay[0], {
                    attributes: !0,
                    characterData: !0,
                    childList: !0
                });
            ...
            stopEventListener() {
                var _this$_controlsObserv;
                null === (_this$_controlsObserv = this._controlsObserver) || void 0 === _this$_controlsObserv || _this$_controlsObserv.disconnect();
            }
            reloadListeners() {
                this.stopEventListener(), this.startEventListener();
            }
        }
        class DisneyContentScript extends class {
            ...
            constructor() {
                const disneyChatApi = new DisneyChatApi, disneyVideoApi = new DisneyVideoApi;
                let pageControls;
                window.teleparty && window.teleparty.pageControls ? (pageControls = window.teleparty.pageControls, 
                console.log("Using existing page controls")) : pageControls = new DisneyPageControls(disneyChatApi);
                super(disneyChatApi, disneyVideoApi, new DisneyVideoEventListener(disneyVideoApi, disneyChatApi, pageControls), pageControls), 
                debug("Disney Content Script");
            }
        }
        window.teleparty && window.teleparty.contentScriptInjected || (window.teleparty || (window.teleparty = {}), 
        window.teleparty.contentScriptInjected = !0, new DisneyContentScript, debug("Listening to messages"));
    })();
})();